...
//   };
 ...
 <View style={[styles.container, { backgroundColor: colors.background }]}>
  ...
  <View>
    <Text style={[styles.headerText, { color: colors.text }]}>My Day</Text>
    <Text style={[styles.headerText, { color: colors.text }]}>{dateString}</Text>
  </View>
    ...
    {isDark ? (
     ...
     <View style={[styles.searchContainer, { backgroundColor: colors.searchBG }]}>
        ...
        <TouchableOpacity style={styles.filterSection} onPress={toggleFilter}>
          <Text style={[styles.filterLabel, {color: colors.text}]}>{filterType}</Text>
          <View style={{ marginLeft: 5 }}>
            {showFilter ? (
              <Entypo name="chevron-thin-up" size={15} color={theme === 'light' ? 'black' :'white'} />
            ) : (
              <Entypo name="chevron-thin-down" size={15} color={theme === 'light' ? 'black' :'white'}  />
            )}
          </View>
        </TouchableOpacity>
        ...
        <View style={styles.searchSection}>
          <Feather name="search" size={15} color={theme === 'light' ? 'black' :'white'} />
          <TextInput
      ...
      {showFilter && (
        ...
        >
          ...
          >
            <Text style={styles.filterText}>Completed ({todos.filter(todo => todo.completed).length})</Text>
          </TouchableOpacity>
          ...
          >
            <Text style={styles.filterText}>Uncompleted ({todos.filter((todo)=>todo.completed === false).length})</Text>
          </TouchableOpacity>
        </View>
      )}

      {/* this is where i have my todo card */}
      <ScrollView
  ...
  {todos.length === 0 ? (
    <View style={styles.emptyState}>
      <Entypo name="emoji-sad" size={40} color={theme === 'light' ? '#888' : colors.text} />
      <Text style={[styles.emptyText, {color: colors.text}]}>No todos available</Text>
      <Text style={[styles.emptySubText, {color: colors.text}]}>Add a task to get started</Text>
    </View>
  ) : (
    filteredTodos.map((todo) => (
    <View key={todo.id} style={[styles.card, { backgroundColor: colors.card }]}>
